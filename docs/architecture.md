...
- `browser-cli` provides `lavish-open` and `lavish-browser-ctl`.
...
A project is either `Zellij(session name, stable tab ID)` or `Standalone`. Labels and working directories are display metadata, never identity. A document is keyed by project plus canonical source path. Duplicate paths in different projects retain separate WebViews but share the globally path-keyed upstream Lavish session and review state.
