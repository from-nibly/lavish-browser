...
def main():
    ...
    memory_valid = all([
        ...
        0 <= memory.get("resume_button_count_after", -1) <= memory.get("resume_button_count_before", 0),
        memory.get("resume_button_count_contract") == "AT-SPI active-subtree count must not increase; model identity proves exactly one resumed source",
        isinstance(memory.get("resume_document_project"), dict),
